...
plot(IPV_ts)
Acf(IPV_ts)
plot(decompose(IPV_ts))
# Based on the Acf graph, the conclusion I could draw is that my time series don't have a trend and seasonality.It's not stationary as well becuase the bar doesn't stay smoothing at a high level. For my plot of time series, for the whole period, there is no upward or downward trend for my data. In other words, remain constant. Through the plot of decomposition, it shows that the seasonality has very little impact on my data
```
...
# The plot shows that the forecast result of the random walk method. Random walk model. It seems like a straight line but the value is actually slightly different from each other.
```
# Seasonal Value
```{r}
